...
上一小节建立的 TCP 通道提供可靠、有序的字节流，而在线系统的业务逻辑通常以一条完整的请求或事件为处理单位，例如文档编辑操作、交易指令、即时消息和游戏输入。
要把接收到的字节恢复为业务逻辑可以处理的消息，应用层协议既要规定消息中的字段如何编码，也要规定每条消息从哪里开始、在哪里结束。
前者由序列化解决，后者由分帧解决。

在线系统中的请求或事件通常是包含类型、标识和参数等字段的结构化对象。
发送前，程序需要先把对象编码成字节序列，接收端再把字节还原成对象，这两个过程分别称为\textbf{序列化}（serialization）和反序列化（deserialization）。
JSON、Protocol Buffers 和自定义二进制格式都可以完成这种转换。

本小节重点讨论消息边界。
设发送端连续发送两条消息：M1（\texttt{attack}）和 M2（\texttt{move}）。
发送端应用程序虽然分别调用两次 \texttt{Write}，TCP 传输的却是连续的字节 \texttt{attackmove}，其中不包含 M1 和 M2 的分隔标记。
接收端第一次 \texttt{Read} 可能读到 \texttt{attackmo}，第二次只读到 \texttt{ve}。
如果程序把每次读取的结果直接当作一条完整消息，就会得到不存在的指令 \texttt{attackmo} 和无法解析的残片 \texttt{ve}。
图~\ref{fig:stream-framing} 的上半部分呈现了这一读取结果。
...
图的上半部分表明，发送端写入消息的次数和接收端读取数据的次数不存在一一对应关系。
多条消息的字节可能在一次读取中合并，这种现象通常称为\textbf{粘包}；一条消息也可能被拆成多次读取，通常称为\textbf{半包}。
这两种现象都说明应用层的消息边界没有在 TCP 字节流中保留。要解释其中的原因，需要进一步理解 TCP 如何处理程序的写入和读取。
...
图~\ref{fig:stream-framing} 的下半部分揭示了一个关键区别：\texttt{Read} 返回的字节数只说明本次取出了多少数据，长度字段才决定一条应用层消息在何处结束。
